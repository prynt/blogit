module Blogit
  module PostsHelper
    ...
    def blog_posts_archive_tag(year_css, month_css, post_css, archive_posts = Post.order("published_at DESC"))
      posts_tree = archive_posts.chunk {|post| post.published_at.year}.map do |year, posts_of_year|
        [year, posts_of_year.chunk {|post| l(post.published_at, format: :plain_month_only) }]
      end
